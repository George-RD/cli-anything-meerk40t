...
- For normal `home`, `physical_home`, and non-`$J` movement, the harness submits the existing MeerK40t console command so native unit parsing, view transforms, active-device selection, and spooler ordering remain authoritative. Success requires the submitted motion job to complete and a subsequently queued native `wait_finish` barrier to complete in the same driver stream. Native spooler queue emptiness by itself is never completion proof; busy/refused, abort, timeout/indeterminate, lost connection, and runtime errors remain structured non-success.
